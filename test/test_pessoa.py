...
Class Pessoa
    __init__
        nome str
        sobrenome str
        dados_obtidos bool (inicial false)
    
    API:
        obter_todos_os_dados -> metodo
            OK
            404
...
class TestPessoa(unittest.TestCase):
    ...
    def setUp(self):
        self.p1 = Pessoa('Humberto','Matondo')    
        self.p2 = Pessoa('Beatriz','Silva')    
    
    def test_pessoa_attr_nome_tem_o_valor_correto(self):
        self.assertEqual(self.p1.nome, 'Humberto') #para saber se os nomes sao iguais
        self.assertEqual(self.p2.nome, 'Beatriz')
        
    def test_pessoa_attr_nome_e_str(self):
        self.assertIsInstance(self.p1.nome, str) # para saber se o nome e instance de STR
        self.assertIsInstance(self.p2.nome, str)
        
    def test_pessoa_attr_sobrenome_tem_o_valor_correto(self):
        self.assertEqual(self.p1.sobrenome, 'Matondo')
    ...
    def test_obter_todos_os_dados_sucesso_OK(self):
        with patch('requests.get') as fake_request:
            ...
            self.assertEqual(self.p2.obter_todos_os_dados(), 'CONECTADO COM SUCESSO')
